// mcp-servers/genapi-dcmstudy/src/dcmstudyService.ts
import { 
  DicomStudy, 
  SearchDicomStudiesParams, 
  SearchDicomStudiesResponse,
  GetDicomStudyResponse
} from './types';
...
export interface GenAPISearchDicomStudiesResponse {
  dicomStudies: Array<{
    name: string;
    displayName: string;
    startTime: string;
...
export interface GenAPIGetDicomStudyResponse {
  name: string;
  displayName: string;
  startTime: string;
  seriesCount: number;
 ...
 */
export async function listDicomStudiesRemote(
  params: SearchDicomStudiesParams & { metadataMask?: string }
): Promise<SearchDicomStudiesResponse> {
  ...
  try {
    ...
    if (params.metadataMask) searchParams.append('metadataMask', params.metadataMask);
    ...
    const data = await response.json() as GenAPISearchDicomStudiesResponse;
    log(`Received ${data.dicomStudies?.length || 0} DICOM studies from GenAPI`);
    
    // Transform the response to our internal format
    const dicomStudies: DicomStudy[] = (data.dicomStudies || []).map(study => ({
 ...
 */
export async function getDicomStudyRemote(
  studyId: string
): Promise<GetDicomStudyResponse> {
  // Get baseUrl and apiKey from environment variables
  const baseUrl = process.env.GENAPI_BASE_URL;
  const apiKey = process.env.GENAPI_API_KEY;
  ...
  try {
    ...
    const data = await response.json() as GenAPIGetDicomStudyResponse;
