...
import { SearchDicomStudiesParams } from './types';
...
export async function main(): Promise<void> {
  ...
  server.registerTool(
    'list-dicom-studies',
    'List DICOM studies in the DS Core Open API system with pagination and filtering support. Note: The filter parameter only supports patient.id and create_time filters. Example filters: patient.id="1234" and create_time >= "2021-01-01T00:00:00Z" and create_time < "2021-01-31T23:59:59Z"',
    {
      pageSize: 'number',
      pageToken: 'string',
      metadataMask: 'string',
    ...
    async (args: any) => {
      const params: SearchDicomStudiesParams = {};
      if (args.pageSize) params.pageSize = args.pageSize;
      if (args.pageToken) params.pageToken = args.pageToken;
      if (args.metadataMask) params.metadataMask = args.metadataMask;
      ...
      try {
        ...
        const result = await listDicomStudiesRemote(params);
  ...
  server.registerTool(
    'get-dicom-study',
    'Get a DICOM study by ID from the DS Core Open API system',
    {
      studyId: 'string'
    },
    async (args: any) => {
      ...
      try {
        ...
        const study = await getDicomStudyRemote(studyId);
