...
import { ListDocumentsParams } from './types';
...
async function main(): Promise<void> {
  ...
  server.registerTool(
    'get-document',
    'Get a document by ID from the DS Core Open API system',
    {
      documentId: 'string'
    },
    async (args: any) => {
  ...
  server.registerTool(
    'list-documents',
    'List documents with pagination and filtering support. Note: The filter parameter only supports patient.id and create_time filters. Example filters: patient.id="1234" and create_time >= "2021-01-01T00:00:00Z" and create_time < "2021-01-31T23:59:59Z"',
    {
      pageSize: 'number',
      pageToken: 'string',
      filter: 'string'
