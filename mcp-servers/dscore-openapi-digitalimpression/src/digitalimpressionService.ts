// mcp-servers/genapi-digitalimpression/src/digitalimpressionService.ts
import { 
  DigitalImpression, 
  ListDigitalImpressionsParams, 
  ListDigitalImpressionsResponse
} from './types';
...
export interface GenAPIListDigitalImpressionsResponse {
  digitalImpressions: Array<{
    name: string;
    contentUri: string;
    createTime: string;
    device?: string;
...
export interface GenAPIGetDigitalImpressionResponse {
  name: string;
  contentUri: string;
  createTime: string;
  device?: string;
 ...
 */
export async function listDigitalImpressionsRemote(
  params: ListDigitalImpressionsParams
): Promise<ListDigitalImpressionsResponse> {
  ...
  try {
    ...
    const data = await response.json() as GenAPIListDigitalImpressionsResponse;
    ...
    const digitalImpressions: DigitalImpression[] = (data.digitalImpressions || []).map(impression => ({
 ...
 */
export async function getDigitalImpressionRemote(
  digitalImpressionId: string
): Promise<DigitalImpression> {
  // Get baseUrl and apiKey from environment variables
  const baseUrl = process.env.GENAPI_BASE_URL;
  const apiKey = process.env.GENAPI_API_KEY;
  ...
  try {
    ...
    const data = await response.json() as GenAPIGetDigitalImpressionResponse;
