...
const genapiService_1 = require("./genapiService");
...
async function main() {
    ...
    server.registerTool('search-patient', 'Search for patients in the DS Core Open API system. Supports filtering by name (with contains modifier), card IDs, and pagination. ' +
        ...
        try {
            ...
            if (name)
                params.name = name;
            const result = await (0, genapiService_1.searchPatientsRemote)(params);
            return result;
        }
        catch (error) {
    ...
    server.registerTool('get-patient', 'Get a patient by ID from the DS Core Open API system', {
        patientId: 'string'
    }, async (args) => {
        ...
        try {
            ...
            const patient = await (0, genapiService_1.getPatientRemote)(patientId);
            return {
                patient: patient
            };
        }
